...
public class PlaceCommentsActivity extends AppCompatActivity {
    ...
    private void getMyComment(){
        ...
        commentService.getMyComment(this,placeId,false).enqueue(new Callback<SingleResponse<Comment>>() {
            @Override
            public void onResponse(Call<SingleResponse<Comment>> call, Response<SingleResponse<Comment>> response) {
                ...
                myComment.addTextChangedListener(new TextWatcher() {
                    ...
                    public void beforeTextChanged(CharSequence s, int start, int count, int after) {
                        if(myComment.getText().length() >= 500){
                            Toast.makeText(PlaceCommentsActivity.this,R.string.character_limit_exceeded,Toast.LENGTH_SHORT).show();
                        }
                    }
                ...
                mySend.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        commentService.uploadComment(PlaceCommentsActivity.this,newComment,placeId).enqueue(new Callback<com.levent.rindex.models.Response>() {
                            @Override
    ...
    private void getCommentsAndList(){
        CommentService commentService = new CommentService();
        int placeId = getIntent().getIntExtra("placeId",0);
        commentService.getComments(PlaceCommentsActivity.this,placeId,false).enqueue(new Callback<ListResponse<Comment>>() {
            @Override
            public void onResponse(Call<ListResponse<Comment>> call, Response<ListResponse<Comment>> response) {
                ...
                CommentsAdapter commentsAdapter = new CommentsAdapter(PlaceCommentsActivity.this,comments,currentUser);
                commentsList.setAdapter(commentsAdapter);
            }

            @Override
            public void onFailure(Call<ListResponse<Comment>> call, Throwable t) {
                Toast.makeText(PlaceCommentsActivity.this,"Failed!",Toast.LENGTH_SHORT).show();
            }
        });
    }
}